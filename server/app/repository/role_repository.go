...
func (r *roleRepository) FindByIds(ctx context.Context, ids []int64) ([]model.Role, error) {
	role := []model.Role{}
	if result := r.DB.Where(ids).Find(&role); result.Error != nil || result.RowsAffected == 0 {
		log.Printf("数据查询失败， ID: %v. 失败原因: %v，影响行数:%d\n", ids, result.Error, result.RowsAffected)
		return role, zerror.NewNotFound("uid", cast.ToString(ids))
	}
	return role, nil
}
...
func (r *roleRepository) FindByWhere(ctx context.Context, where dto.RoleSearchReq) (dto.RoleListResp, error) {
	...
	if err := query.Order("id DESC").Limit(where.PageSize).Offset((where.Page - 1) * where.PageSize).Find(&roles).Error; err != nil {
